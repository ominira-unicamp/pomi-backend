...
async function addCourseFn(
    input: z.infer<typeof IO.addCourse.input>
): Promise<z.infer<typeof IO.addCourse.output>> {
    const {
        path: { sid, cid },
        body
    } = input;
    ...
    if (!curriculum || curriculum.studentId !== sid)
        return {
            404: { description: "Curriculum not found for this student" }
        };

    const courseIdCheck = await zodIds.course.exists.safeParseAsync(
        body.courseId
    );
...
async function updateCourseFn(
    input: z.infer<typeof IO.updateCourse.input>
): Promise<z.infer<typeof IO.updateCourse.output>> {
    const {
        path: { sid, cid, courseId },
        body
    } = input;
...
async function removeCourseFn(
    input: z.infer<typeof IO.removeCourse.input>
): Promise<z.infer<typeof IO.removeCourse.output>> {
    const {
        path: { sid, cid, courseId }
    } = input;
...
router.post(
    "/student/:sid/curricula/:cid/courses",
    buildHandler(IO.addCourse.input, IO.addCourse.output, addCourseFn)
);

router.patch(
    "/student/:sid/curricula/:cid/courses/:courseId",
    buildHandler(IO.updateCourse.input, IO.updateCourse.output, updateCourseFn)
);

router.delete(
    "/student/:sid/curricula/:cid/courses/:courseId",
    buildHandler(IO.removeCourse.input, IO.removeCourse.output, removeCourseFn)
);
