...
async function listFn(
    input: z.infer<typeof IO.list.input>
): Promise<z.infer<typeof IO.list.output>> {
    const {
        path: { sid }
    } = input;
...
async function createFn(
    input: z.infer<typeof IO.create.input>
): Promise<z.infer<typeof IO.create.output>> {
    const {
        path: { sid }
    } = input;
...
async function patchFn(
    input: z.infer<typeof IO.patch.input>
): Promise<z.infer<typeof IO.patch.output>> {
    const {
        path: { sid, id },
        body
    } = input;
    const existing = await prisma.curriculum.findUnique({ where: { id } });
    if (!existing) return { 404: { description: "Curriculum not found" } };
...
async function removeFn(
    input: z.infer<typeof IO.remove.input>
): Promise<z.infer<typeof IO.remove.output>> {
    const {
        path: { sid, id }
    } = input;
    const existing = await prisma.curriculum.findUnique({ where: { id } });
