...
import IO, { CourseBlockInput, CourseBlockOperations, CatalogSpecializationOperations, CatalogLanguageOperations } from './Interface.js';
...
async function createCourseBlocks(
	...
	const { catalogProgramId, catalogSpecializationId, catalogLanguageId } = parentId; 
...
const createFn: HandlerFn<typeof IO.create> = async (ctx, input) => {
	...
	const catalog = await ctx.prisma.catalog.findUnique({ where: { id: body.catalogId } });
	if (!catalog)
		return { 404: { description: "Catalog not found" } };

	const program = await ctx.prisma.program.findUnique({ where: { id: body.programId } });
	if (!program)
	...
	const catalogProgram = await ctx.prisma.$transaction(async (tx) => {
		...
		if (body.courseBlocks) {
			await createCourseBlocks(tx, body.courseBlocks, { catalogProgramId: catalogProgram.id});
		}
...
async function patchCourseBlocks(
	...
	if (ops.set) {
		...
		await createCourseBlocks(tx, ops.set, {catalogProgramId, catalogSpecializationId, catalogLanguageId});
		return;
	}
	...
	if (ops.add) {
		await createCourseBlocks(tx, ops.add, {catalogProgramId, catalogSpecializationId, catalogLanguageId});
	}
	...
	if (ops.upsert) {
		for (const block of ops.upsert) {
			if (block.id) {
				...
				await tx.courseRequirement.deleteMany({ where: { courseBlockId: block.id } });
				if (block.requirements && block.requirements.length > 0) {
					await tx.courseRequirement.createMany({
						data: block.requirements.map(req => ({
				...
				await createCourseBlocks(tx, [block], {catalogProgramId, catalogSpecializationId, catalogLanguageId});
			}
		}
	}
...
async function patchCatalogSpecializations(
	...
	if (ops.set) {
		await tx.catalogSpecialization.deleteMany({ where: { catalogProgramId } });
		for (const spec of ops.set) {
			...
			if (spec.courseBlocks) {
				await createCourseBlocks(tx, spec.courseBlocks, {catalogSpecializationId: catalogSpec.id});
			}
		}
		return;
	}
	...
	if (ops.upsert) {
		for (const spec of ops.upsert) {
			...
			const existingSpec = await tx.catalogSpecialization.findUnique({
				where: { catalogProgramId_specializationId: { catalogProgramId, specializationId: spec.specializationId } }
			});
			if (existingSpec) {
				...
				if (spec.courseBlocks) {
					await createCourseBlocks(tx, spec.courseBlocks, { catalogSpecializationId: existingSpec.id });
				}
			} else {
				const catalogSpec = await tx.catalogSpecialization.create({
					data: {
	...
	if (ops.update) {
		for (const spec of ops.update) {
			if (spec.courseBlocks) {
				...
				if (found) {
					await patchCourseBlocks(tx, spec.courseBlocks, undefined, found.id);
				}
			}
		}
	}
...
async function patchCatalogLanguages(
	...
	if (ops.set) {
		...
		for (const lang of ops.set) {
			...
			if (lang.courseBlocks) {
				await createCourseBlocks(tx, lang.courseBlocks, { catalogLanguageId: catalogLang.id });
			}
		}
		return;
	}
	...
	if (ops.upsert) {
		for (const lang of ops.upsert) {
			...
			if (existingLang) {
				...
				if (lang.courseBlocks) {
					await createCourseBlocks(tx, lang.courseBlocks, { catalogLanguageId: existingLang.id });
				}
			} else {
				const catalogLang = await tx.catalogLanguage.create({
					data: {
	...
	if (ops.update) {
		for (const lang of ops.update) {
			if (lang.courseBlocks) {
				...
				if (found) {
					await patchCourseBlocks(tx, lang.courseBlocks, undefined, undefined, found.id);
				}
			}
		}
	}
...
const patchFn: HandlerFn<typeof IO.patch> = async (ctx, input) => {
	...
	const catalogProgram = 	await ctx.prisma.$transaction(async (tx) => {
		...
		if (body.catalogSpecializations) {
			await patchCatalogSpecializations(tx, body.catalogSpecializations, existing.id);
		}
...
router.get('/catalog-program', buildHandler(IO.list.input, IO.list.output, listFn));
router.get('/catalog-program/:id', buildHandler(IO.get.input, IO.get.output, getFn));
router.post('/catalog-program/:id', buildHandler(IO.create.input, IO.create.output, createFn));
router.patch('/catalog-program/:id', buildHandler(IO.patch.input, IO.patch.output, patchFn));
router.delete('/catalog-program/:id', buildHandler(IO.remove.input, IO.remove.output, removeFn));
